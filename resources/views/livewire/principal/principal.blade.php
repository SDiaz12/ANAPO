<div>
    ...
    @if(auth()->check())
        @if (Auth::user()->activeRole && Auth::user()->activeRole->hasPermissionTo('ver-dashboard-admin'))
            @livewire('principal.inicioadmin')
        @endif

        @if (Auth::user()->activeRole && Auth::user()->activeRole->hasPermissionTo('ver-dashboard-docente')) 
            @livewire('principal.iniciodocente')
        @endif

        @if (Auth::user()->activeRole && Auth::user()->activeRole->hasPermissionTo('ver-dashboard-estudiante'))
            @livewire('principal.inicioestudiante')
        @endif
        {{-- Fallback si ningún permiso coincide --}}
        
        @if (!Auth::user()->activeRole && !Auth::user()->activeRole->hasAnyPermission(['ver-dashboard-admin', 'ver-dashboard-docente', 'ver-dashboard-estudiante']))
            <div class="p-4">
