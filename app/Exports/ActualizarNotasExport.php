    ...
    public function collection()
    {
        return AsignaturaEstudiante::whereHas('asignaturadocente', function ($query) {
            ...
            ->map(function ($item) {
                ...
                $primerparcial = 0;
                $segundoparcial = 0;
                $tercerparcial = 0; // Inicializado en 0 por defecto
                $recuperacion = '';

                // Si hay registro de notas, obtener los valores
                if ($nota) {
                    $primerparcial = $nota->primerparcial ?? 0; // Si es null, usa 0
                    $segundoparcial = $nota->segundoparcial ?? 0; // Si es null, usa 0
                    $tercerparcial = $nota->tercerparcial ?? 0; // Si es null, usa 0
                    $recuperacion = $nota->recuperacion ?? '';
                }

                // Definir los parciales a considerar según el toggle mostrarTercerParcial
                $parciales = [
                    $primerparcial,
                    $segundoparcial
                ];
                ...
                // CAMBIO IMPORTANTE: Ya no filtramos por valores mayores a 0
                // Solo filtramos valores que no sean null para mayor seguridad
                $parcialesFiltrados = array_filter($parciales, function ($value) {
                    return $value !== null;
                });

                // Calcular promedio considerando todos los valores, incluyendo ceros
                ...
                return [
                    ...
                    'tercer_parcial' => $this->mostrarTercerParcial ? $tercerparcial : null, // Solo mostrar tercerparcial si mostrarTercerParcial es true
